...
const requestTypePattern = /^Create|Update|Delete$/;
const serviceTokenPattern = /^arn:(aws(-cn)?):lambda:(.)+:(\d)+:function:(.)+$/;
const stackIdPattern = /^arn:(aws(?:-cn)?):cloudformation:(.+):(\d+):stack\/(.+)$/;

module.exports = event =>
  event &&
  requestTypePattern.test(event.RequestType) &&
  serviceTokenPattern.test(event.ServiceToken) &&
  stackIdPattern.test(event.StackId);
